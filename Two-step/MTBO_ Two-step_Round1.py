...
ker1.variance.constrain_bounded(1e-1, 1e5)
...
# ker3.lengthscale = np.ones(input_dim) * 0.5
...
mse_peak = mse(Y1[:,-1], y1_pred)
print ('Peak rmse: %.4f' % (np.sqrt(mse_peak)))
rsquared_peak = r2_score(Y1[:,-1], y1_pred)
print ('Peak R^2: %.4f' % (rsquared_peak))
sprman_peak = spearmanr(Y1[:,-1], y1_pred)
print ('Peak spearman: %.4f' % (sprman_peak[0]))
...
df_Xnew.to_csv('1st round suggestions.csv')
...
#             c_plt1 = ax.contourf(x1, x2, y,cmap='plasma',extend='both')
        colorbar_offset = [30, 20, 0]
        for ax, c_offset, y in zip(axes,colorbar_offset,
                           [y_max2, y_mean2, y_min2]):
            c_plt1 = ax.contourf(x1, x2, y,levels = np.arange(31)+c_offset,cmap='plasma',extend='both')
            cbar = fig.colorbar(c_plt1, ax= ax)
            cbar.ax.tick_params(labelsize=fs*0.8)
...
#             c_plt1 = ax.contourf(x1, x2, y,cmap='plasma',extend='both')
        ...
        for ax, c_offset, y in zip(axes,colorbar_offset,
                           [y_max2, y_mean2, y_min2]):
            c_plt1 = ax.contourf(x1, x2, y,levels = np.arange(21)*2+c_offset,cmap='plasma',extend='both')
            cbar = fig.colorbar(c_plt1, ax= ax)
            cbar.ax.tick_params(labelsize=fs*0.8)
            ax.scatter(x_denormalizer(X2)[:, ind1], 
                       x_denormalizer(X2)[:, ind2], 
    ...
    for j in range(input_dim-i-1):
        ...
        for x1 in np.linspace(0, 1, n_steps):
            for x2 in np.linspace(0, 1, n_steps):
                ...
                x1x2y_pred.append([x1_org, x2_org, np.max(y1), np.mean(y1), np.min(y1),
                                   np.max(y2), np.mean(y2), np.min(y2),
        ...
        y_max2 = np.array(x1x2y_pred, dtype=object)[:,5]
        y_mean2 = np.array(x1x2y_pred, dtype=object)[:,6]
        y_min2 = np.array(x1x2y_pred, dtype=object)[:,7]
        cons_max = np.array(x1x2y_pred, dtype=object)[:,8]
        cons_mean = np.array(x1x2y_pred, dtype=object)[:,9]
        cons_min = np.array(x1x2y_pred, dtype=object)[:,10]
        
        tolerances = np.array([0.2,0.5]) 
        absolutes = [False, False]
        goals = ['min', 'min'] 
        chimera = Chimera(tolerances=tolerances, absolutes=absolutes, goals=goals)
        acq = np.array([y_min1.flatten(), y_min2.flatten()])
        scalarized = chimera.scalarize(acq.T)
        raw_acq= (1-scalarized).reshape(n_steps, n_steps)   
#         raw_acq= scalarized.reshape(n_steps, n_steps) 
        cons_acq = ((1-scalarized)*cons_max/max(cons_max.flatten())).reshape(n_steps, n_steps)
#         cons_acq = ((1-scalarized)*cons_max).reshape(n_steps, n_steps)
        cons_max = cons_max.reshape(n_steps, n_steps)
...
#             c_plt1 = ax.contourf(x1, x2, y,cmap='coolwarm',extend='both')
        ...
        for ax, c_offset, y in zip(axes,colorbar_offset,
                   [cons_max, raw_acq, cons_acq]):
            c_plt1 = ax.contourf(x1, x2, y,levels = np.arange(20)*0.05+c_offset,cmap='coolwarm',extend='both')
            cbar = fig.colorbar(c_plt1, ax= ax)
            cbar.ax.tick_params(labelsize=fs*0.8)
            ax.scatter(x_denormalizer(X3)[:, ind1], 
                       x_denormalizer(X3)[:, ind2], 
        ...
        axes[0].set_title('constraint func', pad = title_pad,fontsize =  fs)
        axes[1].set_title('raw acq', pad = title_pad,fontsize =  fs)
        axes[2].set_title('final acq', pad = title_pad,fontsize =  fs)
