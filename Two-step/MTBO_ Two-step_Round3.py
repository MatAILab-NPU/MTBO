...
# axes[0].hlines([20], 45,59, linestyles="-", colors='red',linewidth=2,label= 'tol')
...
axes[0].set_xlim(-1, 95)
axes[0].set_xticks(np.arange(0,95,5))
...
# axes[1].hlines([30], 45,59, linestyles="-", colors='r',linewidth=2,label= 'tol')
...
axes[1].set_xlim(-1, 95)
axes[1].set_xticks(np.arange(0,95,5))
...
# axes[2].hlines([yc_offset], -5,45, linestyles="--", colors='red',linewidth=2)

axes[2].set_ylabel('Current Best Film', fontsize = 20)
axes[2].set_xlabel('Process Condition', fontsize = 20)

axes[2].set_ylim(-.1, 1.3)
axes[2].set_xlim(-1, 95)
axes[2].set_xticks(np.arange(0,95,5))
...
axes[0].set_xticks(np.arange(0,76,5))
axes[0].legend(fontsize = fs*0.7)
...
axes[1].set_xlabel('Process Condition', fontsize = 20)
axes[1].set_ylim(-20, 100)
axes[1].set_xlim(-1, 75)
axes[1].set_xticks(np.arange(0,76,5))
axes[1].legend(fontsize = fs*0.7,loc=1)
...
axes[2].set_ylim(-.1, 1.3)
axes[2].set_xlim(-1, 75)
axes[2].set_xticks(np.arange(0,76,5))
axes[2].legend(fontsize = fs*0.7)
...
#             c_plt1 = ax.contourf(x1, x2, y,cmap='viridis',extend='both')
        ...
        for ax, c_offset, y in zip(axes,colorbar_offset,
                           [y_max2, y_mean2, y_min2]):
            c_plt1 = ax.contourf(x1, x2, y,levels = np.arange(20)*0.05+c_offset,cmap='viridis',extend = 'both')
...
#             c_plt1 = ax.contourf(x1, x2, y,cmap='plasma',extend='both')
        colorbar_offset = [40, 30, 0]
        for ax, c_offset, y in zip(axes,colorbar_offset,
                           [y_max2, y_mean2, y_min2]):
            c_plt1 = ax.contourf(x1, x2, y,levels = np.arange(31)*0.5+c_offset,cmap='plasma',extend='both')
            cbar = fig.colorbar(c_plt1, ax= ax)
            cbar.ax.tick_params(labelsize=fs*0.8)
...
#             c_plt1 = ax.contourf(x1, x2, y,cmap='plasma',extend='both')
        ...
        for ax, c_offset, y in zip(axes,colorbar_offset,
                           [y_max2, y_mean2, y_min2]):
            c_plt1 = ax.contourf(x1, x2, y/max(y_max2.flatten()),levels =np.arange(20)*0.05+c_offset,cmap='viridis',extend='both')
            cbar = fig.colorbar(c_plt1, ax= ax)
            cbar.ax.tick_params(labelsize=fs*0.8)
            ax.scatter(x_denormalizer(X3)[:, ind1], 
                       x_denormalizer(X3)[:, ind2], 
